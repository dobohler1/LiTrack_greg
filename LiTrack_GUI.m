...
% Last Modified by GUIDE v2.5 12-Sep-2016 12:07:42
...
handles.time=get(handles.time_edit, 'String');
...
if run_ok
    ...
    drawnow
    handles.es = get(handles.es_checkbox, 'Value');
    if handles.es
        
        %Get cost function
        [res_I, res_E] = LiTrack_imageConverter(handles, 1);
        cost=res_I+res_E;
        ...
        low_lims=-360+zeros(1,np);
        high_lims=360+zeros(1,np);
        
        % Dithering frequencies evenly spread between w0 and 1.8w0
        w0=2000;
        w=zeros(1,np);
        for j=1:np;
            w(j)=w0+(j-1)*0.8*w0/(np-1);
        end
        ...
        % Keep track of parameters and costs
        ESparams=zeros(np,ESsteps);
        EScost=zeros(1,np);
...
%time_edit = get(handles.time_edit, 'String');

% --- Executes on button press in Load.
function Load_Callback(hObject, eventdata, handles)
time = get(handles.time_edit, 'String');
%LiTrack_Loader(1, time,'paramFileOutFn','Something_else(1).mat')
pars=LiTrack_Loader(1, time,'paramFileOutFn','PV_history.mat','partstart',6);

%beamline components pars.beamline;
handles.time = time
guidata(hObject, handles);
...
% --- Executes on button press in upload_particles.
function upload_particles_Callback(hObject, eventdata, handles)
time = get(handles.time_edit, 'String');
string_bean(1, time)
handles.time = time;
guidata(hObject, handles);
...
% --- Executes on button press in es_checkbox.
function es_checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to es_checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of es_checkbox
